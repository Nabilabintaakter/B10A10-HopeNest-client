...
const CampaignCard = ({ campaign }) => {
    return (
        <Link to={`/campaigns/${campaign?._id}`} className="bg-white rounded-md overflow-hidden hover:shadow-[0_4px_8px_0_rgba(0,0,0,0.5)] dark:hover:shadow-[0_0px_10px_0_rgba(255,255,255,0.9)] transition-all duration-300 flex flex-col group">
    {/* Thumbnail Section */}
    <div className="relative w-full h-36 sm:h-44">
        ...
        />
        <div
            className="absolute top-2 md:top-3 left-2 md:left-3 bg-orange-100 text-orange-600 px-3  border-[1px] border-orange-500 rounded-full text-xs font-medium cursor-pointer"
        >
            {campaign?.type}
        </div>
    </div>
    ...
    <div className="p-2 md:p-3 flex flex-col justify-between flex-grow">
        ...
        <div className="flex justify-between items-center text-gray-700 mt-2">
            ...
            <Link
                ...
                className="w-full block bg-blue-600 hover:bg-blue-700 text-white py-1 md:py-2 rounded-md text-xs md:text-base text-center transition-colors duration-300 mt-2"
            >
                See More
            </Link>
        </div>
